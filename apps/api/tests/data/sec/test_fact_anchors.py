...
# Two rows carrying the same printed figure, the earlier one in prose. The
# browser resolves a text fragment to the FIRST match, so only a figure whose
# first printing is its own may be highlighted.
HIGHLIGHT_BODY = """
<table><tr><td>Cash generated by operating activities</td>
<td><ix:nonFraction unitRef="usd" contextRef="c-1" name="us-gaap:NetCashProvidedByUsedInOperatingActivities"
 format="ixt:num-dot-decimal" scale="6" id="f-307">111,482</ix:nonFraction></td></tr>
<tr><td>Payments for property and equipment</td><td>(<ix:nonFraction unitRef="usd" contextRef="c-1"
 sign="-" name="us-gaap:PaymentsToAcquirePropertyPlantAndEquipment"
 format="ixt:num-dot-decimal" scale="6" id="f-319">12,715</ix:nonFraction>)</td></tr></table>
<p>Earlier discussion already mentioned 4,832 in the narrative.</p>
<table><tr><td>Long-term debt</td>
<td><ix:nonFraction unitRef="usd" contextRef="c-1" name="us-gaap:LongTermDebt"
 format="ixt:num-dot-decimal" scale="6" id="f-940">4,832</ix:nonFraction></td></tr></table>
</body></html>
"""


def highlight_of(anchor, content=HEADER + HIGHLIGHT_BODY):
    for facts in filing_fact_index(document(content), CIK).values():
        for fact in facts:
...
def test_offers_the_figure_as_the_filing_prints_it():
    assert highlight_of("f-307") == "111,482"


def test_a_bracketed_negative_still_offers_its_digits():
    """The filing prints "(12,715)"; the figure a reader looks for is 12,715."""
    assert highlight_of("f-319") == "12,715"


def test_refuses_a_figure_the_prose_printed_first():
    """A text fragment lands on the first match. Highlighting this one would mark
    the sentence above the table instead of the row."""
    assert highlight_of("f-940") is None


def test_a_cell_boundary_is_not_a_word_boundary():
    """Without block breaks the visible text reads "activities111,482", and the
    figure looks like the tail of a word rather than a number of its own."""
    assert highlight_of("f-307") is not None


def test_the_highlight_travels_with_the_anchor_that_earned_it():
    annotated = annotate_filing_anchors(
        result(reference(OCF, 111_482_000_000.0), reference(CAPEX, 12_715_000_000.0)),
        document(HEADER + HIGHLIGHT_BODY),
    )
    refs = annotated.facts["free_cash_flow"][0].evidence
    assert [(r.filing_anchor, r.filing_highlight) for r in refs] == [
        ("f-307", "111,482"), ("f-319", "12,715"),
    ]


def test_an_unanchored_reference_carries_no_highlight():
    annotated = annotate_filing_anchors(
        result(reference(OCF, 99_000_000_000.0)), document(HEADER + HIGHLIGHT_BODY),
    )
    reference_out = annotated.facts["free_cash_flow"][0].evidence[0]
    assert (reference_out.filing_anchor, reference_out.filing_highlight) == (None, None)
