...
def test_slow_primary_timeout_reaches_fallback_with_reserved_budget(wrapped, caplog):
    ...
    assert [timeout for _, timeout in attempts] == [30.0, 30.0]
    ...
    assert failure.elapsed_ms == 30_000
...
def test_call_ids_and_budgets_are_per_generation_not_client_global(caplog):
    ...
    assert all(r.attempt_number == 1 and r.budget_remaining_seconds == 60 for r in starts)
